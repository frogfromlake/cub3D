...
uint32_t	get_pixel(t_data *game, int x, int y)
{
	...
	h = game->wall_heights[x] + (game->mouse.pitch / game->wall_heights[x]);
	fac = (0.5 * (HEIGHT - h)) + game->mouse.pitch;
	if (y > fac && y < fac + h)
	{
		y_rat = (float)(y - fac) / h;
	...
	else if (y > HEIGHT * 0.5 + game->mouse.pitch)
