...
/*   Updated: 2022/07/08 22:23:46 by gsiddiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	loading_color_params(t_data *data, int canvas, char *str, int i)
{
	if (canvas == 1)
	{
		data->map.f_colour = parsing_color(data, str, i);
		data->map.f = true;
	}
	if (canvas == 2)
	{
		data->map.c_colour = parsing_color(data, str, i);
		data->map.c = true;
	}
}
...
void	loading_map(t_data *data)
{
	...
	while (line != EOF)
	{
		...
		{
			data->map.map[i] = cub_strdup(data, str);
			i++;
			free(str);
		}
		else
